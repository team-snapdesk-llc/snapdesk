...
class BystanderTicketBox extends Component {
  ...
  render () {
    ...
    if (this.props.ticket.status === 'active') {
      ...
      buttons = (
        <span>
          <Button onClick={() => store.dispatch(ticketActions.updateTicketSocket(props.socket, props.ticket.messageId, 'pending', (props.ticket.mentorId || null)))} type="button" className="btn btn-success">Accept</Button>
      ...
      buttons = (
        <span>
          <Button disabled type="button" className="btn btn-success">Accept (disabled)</Button>
          <Button onClick={() => store.dispatch(ticketActions.updateTicketSocket(props.socket, props.ticket.messageId, 'active', (props.ticket.mentorId || null)))} type="button" className="btn btn-warning">Cancel</Button>
       </span>
        )
    }
